...
foreach ($required in @('src\OpenSynapse.PowerShell', 'OpenSynapse-2.5.0.zip', 'Compress-Archive')) {
...
foreach ($commandPath in @(
    ...
    if ((Get-Content -Raw -LiteralPath $commandPath).IndexOf('OpenSynapse 2.5.0', [StringComparison]::Ordinal) -lt 0) {
