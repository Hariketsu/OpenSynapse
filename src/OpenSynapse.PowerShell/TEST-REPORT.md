...
- 新增外屏模式指纹回归，覆盖分辨率、刷新率、色深、位置和方向；模拟内屏切换后外屏保持不变，并确认外屏漂移会使验证失败。
- 当前仅外屏活动的拓扑下，内屏动态刷新安全跳过，不对外屏执行替代写入；未主动切换真实外屏刷新率，以避免重现已知唤醒风险。
- 四个档位的交流电显示关闭均为 900 秒、锁屏显示关闭为 900 秒、睡眠为 10800 秒、休眠为禁用；电池侧既有策略保持不变。
- PowerShell 24/24 项定义与运行时测试通过；.NET Core 39/39、Agent 53/53 测试通过；安装器定义、PowerShell 解析、原生 C# 编译与差异检查通过。
- 版本 2.5.2；未修改 GPU/MUX 选择逻辑，也未更改 OEM 异构核心阈值、EC、风扇、TGP 或 DeathAdder V3 Pro HID 控制路径。
