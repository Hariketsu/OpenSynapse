...
if ($config.Version -ne 10 -or -not $config.AdaptiveQuietBrightness -or -not $config.AdaptiveQuietCpu -or
    $config.ProcessMaintenanceSeconds -ne 180) {
...
$cpuTargets = @(
    ...
    (Resolve-QuietCpuMaxPercent $config $batteryMedium),
    (Resolve-QuietCpuMaxPercent $config $batteryLow)
)
if (($cpuTargets -join ',') -ne '75,65,60') { throw "Adaptive CPU curve is incorrect: $($cpuTargets -join ',')" }

$script:RecordedCpuWrites = New-Object Collections.Generic.List[int]
...
function Set-PlanValue {
    ...
    if ($PlanGuid -ne '11111111-1111-1111-1111-111111111111' -or $Source -ne 'DC' -or
        $Subgroup -ne $script:Guids.Processor -or $Setting -ne $script:Guids.ProcessorMaximum) {
        throw 'Adaptive CPU wrote an unexpected power setting.'
    }
    $script:RecordedCpuWrites.Add($Value)
}
...
$null = Apply-QuietDynamicCpuPolicy $config $quietState $batteryHigh
$null = Apply-QuietDynamicCpuPolicy $config $quietState $batteryMedium
$null = Apply-QuietDynamicCpuPolicy $config $quietState $batteryLow
if (($script:RecordedCpuWrites -join ',') -ne '75,65,60' -or $script:RecordedReactivations -ne 3) {
