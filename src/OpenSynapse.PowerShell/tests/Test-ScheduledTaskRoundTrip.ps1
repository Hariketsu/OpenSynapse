...
$action = New-ScheduledTaskAction -Execute $exe -Argument ('-NoProfile -WindowStyle Hidden -STA -ExecutionPolicy Bypass -File "{0}" -Mode Run' -f $installed) -WorkingDirectory (Split-Path -Parent $installed)
...
$result = [pscustomobject]@{ Result = 'PASS'; Registered = $registered; HighestRunLevel = $true; ProtectedActionPath = $true; LogonDelay = 'PT30S'; TaskRemoved = $removed; CompletedAt = (Get-Date).ToString('o') }
