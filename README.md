...
| --- | --- | --- |
| Windows policies | Adapter-aware Auto, Hyper, Balance, Eco, and locked Experiment selection; power plans; internal-panel-only refresh rate; Advanced Color/HDR; per-display brightness; display scaling; optional wake-device control | 2.5.2 external-refresh isolation and AC timeout policy tested |
...
OpenSynapse never manages external-display refresh rates. The internal-panel `Auto` policy uses native 60–240 Hz dynamic refresh on battery, USB-C PD, and unverified AC, then returns to fixed 240 Hz after high-power AC is verified. Manual Eco 60 Hz and fixed 240 Hz remain available and return to `Auto` when external power is newly connected. When the internal panel is inactive, refresh work is deferred without touching active external displays. If Windows accepts neither the dynamic request nor its verification, only the internal panel falls back to Eco 60 Hz. Every profile uses a 15-minute AC display timeout, a separate 15-minute AC console-lock display timeout, and a three-hour AC standby timeout.
